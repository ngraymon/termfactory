...
def generate_eT_taylor_expansion(maximum_eT_rank=2, eT_taylor_max_order=3):
    ...
    Each of the terms in that sum (T, T^2, ...) is represented by a list inside the returned list.
    There is no 1 term in the eT case.
    ...
    eT_taylor_expansion[0] = T.operator_list                            # T term
    ...
    """
    for n in range(1, eT_taylor_max_order+1):
...
def _build_eT_z_latex_prefactor(t_list, h, z_left, z_right, simplify_flag=True):
    ...
    # account for prefactor from Hamiltonian term `h` and each Z
    # x = len(set([t for t in t_list if isinstance(t, connected_t_operator_namedtuple)]))
    x = len([t for t in t_list if isinstance(t, connected_t_operator_namedtuple)])
