...
# local imports
from .context import code_full_cc as cfcc
...
class Test_gen_full_cc_py_eqns:
    ...
    def test_multiple_perms_logic(self):
        ...
        expected_result = ([(0, 1), (1, 0)], {fcc.disconnected_namedtuple(m_h=0, n_h=0, m_o=0, n_o=1): 1, fcc.connected_namedtuple(m_h=1, n_h=0, m_o=0, n_o=1): 1})
        assert function_output == expected_result
    ...
    def test_write_cc_einsum_python_from_list(self):
        ...
        function_output = cfcc._write_cc_einsum_python_from_list(rank, truncations, t_term_list, trunc_obj_name='truncation')
        expected_result = ['if truncation.singles:', "    R += np.einsum('ac, cbz -> abz', h_args[(0, 0)], t_args[(1, 0)])", '', 'if truncation.at_least_linear:']
        assert function_output == expected_result
    ...
    def test_write_cc_einsum_python_from_list_zero_dc_case(self):
        ...
        function_output = cfcc._write_cc_einsum_python_from_list(rank, truncations, t_term_list, trunc_obj_name='truncation')
        expected_result = ['R += h_args[(1, 0)]', '', 'if truncation.at_least_linear:']
        assert function_output == expected_result
    ...
    def test_write_cc_einsum_python_from_list_0_indices(self):
        ...
        function_output = cfcc._write_cc_einsum_python_from_list(rank, truncations, t_term_list, trunc_obj_name='truncation')
        expected_result = ['R += h_args[(0, 0)]', '', 'if truncation.at_least_linear:', '    if truncation.singles:', "        R += np.einsum('aci, cbi -> ab', h_args[(0, 1)], t_args[(1, 0)])"]
        assert function_output == expected_result
    ...
    def test_write_cc_einsum_python_from_list_single_unique_key(self):
        ...
        function_output = cfcc._write_cc_einsum_python_from_list(rank, truncations, t_term_list, trunc_obj_name='truncation')
        expected_result = ['if truncation.singles:', '    R += 1/(factorial(2)) * (', "        np.einsum('ac, cdz, dby -> abzy', h_args[(0, 0)], t_args[(1, 0)], t_args[(1, 0)]) +", "        np.einsum('ac, cdy, dbz -> abzy', h_args[(0, 0)], t_args[(1, 0)], t_args[(1, 0)])", '    )', '', 'if truncation.at_least_linear:', '    if truncation.singles:', '        R += (', "            np.einsum('acz, cby -> abzy', h_args[(1, 0)], t_args[(1, 0)]) +", "            np.einsum('acz, cby -> abzy', h_args[(1, 0)], t_args[(1, 0)])", '        )', '        R += 1/(factorial(2)) * (', "            np.einsum('aciz, cdy, dbi -> abzy', h_args[(1, 1)], t_args[(1, 0)], t_args[(1, 0)]) +", "            np.einsum('aciz, cdi, dby -> abzy', h_args[(1, 1)], t_args[(1, 0)], t_args[(1, 0)])", '        )', '    if truncation.doubles:', '        R += 1/(factorial(2)) * (', "            np.einsum('aci, cdz, dbiy -> abzy', h_args[(0, 1)], t_args[(1, 0)], t_args[(2, 0)]) +", "            np.einsum('aci, cdiy, dbz -> abzy', h_args[(0, 1)], t_args[(2, 0)], t_args[(1, 0)])", '        )', '', 'if truncation.at_least_quadratic:']
        assert function_output == expected_result

    def test_generate_full_cc_einsums(self):
        omega_term = fcc.general_operator_namedtuple(name='b', rank=1, m=0, n=1)
        truncations = [1, 1, 1, 1]
        function_output = cfcc._generate_full_cc_einsums(omega_term, truncations, only_ground_state=False, opt_einsum=False)
        expected_result = [['R += h_args[(1, 0)]', '', 'if truncation.at_least_linear:'], ['pass  # no valid terms here'], ['if truncation.singles:', "    R += np.einsum('ac, cbz -> abz', h_args[(0, 0)], t_args[(1, 0)])", '', 'if truncation.at_least_linear:']]
        assert function_output == expected_result

    def test_generate_full_cc_compute_function(self):
        omega_term = fcc.general_operator_namedtuple(name='b', rank=1, m=0, n=1)
        truncations = [1, 1, 1, 1]
        function_output = cfcc._generate_full_cc_compute_function(omega_term, truncations, only_ground_state=False, opt_einsum=False)
        expected_result = '\ndef add_m0_n1_fully_connected_terms(R, ansatz, truncation, h_args, t_args):\n    """Compute the operator(name=\'b\', rank=1, m=0, n=1) fully_connected terms."""\n    if ansatz.ground_state:\n        R += h_args[(1, 0)]\n\n        if truncation.at_least_linear:\n    else:\n        R += h_args[(1, 0)]\n\n        if truncation.at_least_linear:\n    return\n\n\ndef add_m0_n1_linked_disconnected_terms(R, ansatz, truncation, h_args, t_args):\n    """Compute the operator(name=\'b\', rank=1, m=0, n=1) linked_disconnected terms."""\n    if ansatz.ground_state:\n        pass  # no valid terms here\n    else:\n        pass  # no valid terms here\n    return\n\n\ndef add_m0_n1_unlinked_disconnected_terms(R, ansatz, truncation, h_args, t_args):\n    """Compute the operator(name=\'b\', rank=1, m=0, n=1) unlinked_disconnected terms."""\n    if ansatz.ground_state:\n        if truncation.singles:\n            R += np.einsum(\'ac, cbz -> abz\', h_args[(0, 0)], t_args[(1, 0)])\n\n        if truncation.at_least_linear:\n    else:\n        if truncation.singles:\n            R += np.einsum(\'ac, cbz -> abz\', h_args[(0, 0)], t_args[(1, 0)])\n\n        if truncation.at_least_linear:\n    return\n\n'
        assert function_output == expected_result

    def test_generate_full_cc_compute_function_opt_einsum(self):
        omega_term = fcc.general_operator_namedtuple(name='b', rank=1, m=0, n=1)
        truncations = [1, 1, 1, 1]
        function_output = cfcc._generate_full_cc_compute_function(omega_term, truncations, only_ground_state=False, opt_einsum=True)
        expected_result = '\ndef add_m0_n1_fully_connected_terms_optimized(R, ansatz, truncation, h_args, t_args, opt_paths):\n    """Optimized calculation of the of the operator(name=\'b\', rank=1, m=0, n=1) fully_connected terms."""\n\n    if ansatz.ground_state:\n        R += h_args[(1, 0)]\n        \n        if truncation.at_least_linear:\n    else:\n        R += h_args[(1, 0)]\n        \n        if truncation.at_least_linear:\n    return\n\n\ndef add_m0_n1_linked_disconnected_terms_optimized(R, ansatz, truncation, h_args, t_args, opt_paths):\n    """Optimized calculation of the of the operator(name=\'b\', rank=1, m=0, n=1) linked_disconnected terms."""\n\n    if ansatz.ground_state:\n        pass  # no valid terms here\n    else:\n        pass  # no valid terms here\n    return\n\n\ndef add_m0_n1_unlinked_disconnected_terms_optimized(R, ansatz, truncation, h_args, t_args, opt_paths):\n    """Optimized calculation of the of the operator(name=\'b\', rank=1, m=0, n=1) unlinked_disconnected terms."""\n\n    if ansatz.ground_state:\n        if truncation.singles:\n            R += np.einsum(\'ac, cbz -> abz\', h_args[(0, 0)], t_args[(1, 0)])\n        \n        if truncation.at_least_linear:\n    else:\n        if truncation.singles:\n            R += np.einsum(\'ac, cbz -> abz\', h_args[(0, 0)], t_args[(1, 0)])\n        \n        if truncation.at_least_linear:\n    return\n\n'
        assert function_output == expected_result
