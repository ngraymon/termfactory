        ...
        if truncation.h_at_least_linear:
            if truncation.t_singles:
                ...
                if truncation.z_at_least_quartic:
                    ...
                    R += (1 / 6) * np.einsum('i, j, k, aclz, cijkl -> az', t_args[(0, 1)], t_args[(0, 1)], t_args[(0, 1)], h_args[(1, 1)], z_args[(4, 0)])
    ...
    if ansatz.ground_state:
        ...
        if truncation.h_at_least_linear:
            if truncation.t_singles:
                ...
                if truncation.z_at_least_quartic:
                    R += (1 / 2) * np.einsum('i, acjz, cijyx -> azyx', t_args[(0, 1)], h_args[(1, 1)], z_args[(4, 0)])
                    R += (1 / 4) * np.einsum('i, j, acz, cijyx -> azyx', t_args[(0, 1)], t_args[(0, 1)], h_args[(1, 0)], z_args[(4, 0)])
                    R += (1 / 6) * (
                        np.einsum('i, acij, cjzyx -> azyx', t_args[(0, 1)], h_args[(1, 1)], z_args[(4, 0)]) +
                        np.einsum('i, j, acj, cizyx -> azyx', t_args[(0, 1)], t_args[(0, 1)], h_args[(1, 0)], z_args[(4, 0)])
                    )
        ...
        if truncation.h_at_least_quadratic:
            if truncation.t_singles:
                ...
                if truncation.z_at_least_quartic:
                    R += (1 / 12) * (
                        np.einsum('i, j, k, acjk, cizyx -> azyx', t_args[(0, 1)], t_args[(0, 1)], t_args[(0, 1)], h_args[(2, 0)], z_args[(4, 0)]) +
                        np.einsum('i, j, k, aczy, cijkx -> azyx', t_args[(0, 1)], t_args[(0, 1)], t_args[(0, 1)], h_args[(2, 0)], z_args[(4, 0)])
                    )
                    R += (1 / 4) * np.einsum('i, j, k, ackz, cijyx -> azyx', t_args[(0, 1)], t_args[(0, 1)], t_args[(0, 1)], h_args[(2, 0)], z_args[(4, 0)])
    else:
        raise Exception('Hot Band amplitudes not implemented!')
...
def add_m0_n4_HZ_terms(R, ansatz, truncation, t_args, h_args, z_args):
    ...
    if ansatz.ground_state:
        if truncation.h_at_least_linear:
            ...
            if truncation.z_at_least_quartic:
                R += (1 / 6) * np.einsum('aciz, ciyxw -> azyxw', h_args[(1, 1)], z_args[(4, 0)])

        if truncation.h_at_least_quadratic:
            if truncation.z_at_least_quadratic:
                R += (1 / 4) * np.einsum('aczy, cxw -> azyxw', h_args[(2, 0)], z_args[(2, 0)])
    else:
        raise Exception('Hot Band amplitudes not implemented!')
...
def add_m0_n4_eT_HZ_terms(R, ansatz, truncation, t_args, h_args, z_args):
    ...
    if ansatz.ground_state:
        ...
        if truncation.h_at_least_quadratic:
            if truncation.t_singles:
                if truncation.z_at_least_cubic:
                    R += (1 / 4) * np.einsum('i, aczy, cixw -> azyxw', t_args[(0, 1)], h_args[(2, 0)], z_args[(3, 0)])
                    R += (1 / 6) * np.einsum('i, aciz, cyxw -> azyxw', t_args[(0, 1)], h_args[(2, 0)], z_args[(3, 0)])
                if truncation.z_at_least_quartic:
                    R += (1 / 48) * np.einsum('i, j, acij, czyxw -> azyxw', t_args[(0, 1)], t_args[(0, 1)], h_args[(2, 0)], z_args[(4, 0)])
                    R += (1 / 6) * np.einsum('i, j, acjz, ciyxw -> azyxw', t_args[(0, 1)], t_args[(0, 1)], h_args[(2, 0)], z_args[(4, 0)])
                    R += (1 / 8) * np.einsum('i, j, aczy, cijxw -> azyxw', t_args[(0, 1)], t_args[(0, 1)], h_args[(2, 0)], z_args[(4, 0)])
    else:
        raise Exception('Hot Band amplitudes not implemented!')
